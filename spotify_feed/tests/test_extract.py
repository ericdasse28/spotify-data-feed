...
def test_request_recently_played_tracks_after_a_date():
    data = get_recent_tracks(after="2022-05-03", limit=10)
    reference_date = datetime.datetime.strptime("2022-05-03", "%Y-%m-%d")
    assert data, "Unexpectedly get no songs before May 3rd, 2022"
    assert (
        "error" not in data.keys()
...
def test_request_use_cache():
    ...
    _ = request_tracks_after(after="2022-05-03", limit=10)
    response_2 = request_tracks_after(after="2022-05-03", limit=10)
